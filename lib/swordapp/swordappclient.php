...
class SWORDAPPClient {
	...
	function deposit($sac_url, $sac_u, $sac_p, $sac_obo, $sac_fname, 
		...
		curl_setopt($sac_curl, CURLOPT_POST, true);
                curl_setopt($sac_curl, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
		...
		if (($sac_status >= 200) || ($sac_status < 300)) {
			try {
			    ...
			    throw new Exception("Error parsing response entry (" . $e->getMessage() . ") The Remote server returned HTTP status {$sac_status}");
			}
		} else {
			try {
				// Parse the result
