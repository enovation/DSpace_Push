 ...
 */
	...
	$.mcDropDownMenu = function(el, list, options, indexid){
		...
		$divInput = $('<div class="mcdropdown_'+indexid+' mcdropdown" name="mcdropdown"><a href="#" tabindex="-1">UU</a><input type="hidden" name="' + (el.name || el.id) + '" id="' + (el.id || el.name) + '" /></div>')
		...
		this.setValue = function (value, skipCallback){
			// update the hidden value
                        ...
                        if ($hidden.attr('id') != 'id_isced_'+$('#current_menu').val()) {
                            return false;
                        }
